...
from .models import categoria, subcategoria, roles, usuario, domiciliario, producto, Calificacion, carritocompras
...
def devoluciones(request):
    # Buscar items cuyo estado sea "Cancelado" (case-insensitive)
    items = carritocompras.objects.filter(estado__iexact='cancelado').select_related('usuario')

    # Si no hay ninguno, pasamos una lista vacía
    if not items.exists():
        return render(request, 'pedidos/devoluciones.html', {'pedidos_cancelados': []})

    # Agrupar por codigo_pedido y sumar totales, tomar fecha máxima (fecha de compra)
    agregados = items.values('codigo_pedido').annotate(
        total=Sum('precio_total'),
        fecha=Max('fecha_compra'),
    ).order_by('-fecha')
    ...
    pedidos = []
    for a in agregados:
        codigo = a['codigo_pedido']
        primer_item = items.filter(codigo_pedido=codigo).first()
        if primer_item and primer_item.usuario:
            cliente = f"{primer_item.usuario.PrimerNombre} {primer_item.usuario.PrimerApellido}"
        else:
    ...
    return render(request, 'pedidos/devoluciones.html', {'pedidos_cancelados': pedidos})
